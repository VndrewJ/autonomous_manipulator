...
int main(int argc, char** argv) {
    ...
    while (ros::ok() && !viewer->wasStopped()) {
        ...
        if (elapsed_time.count() >= 6.0) {
            ROS_INFO("Capture stopped after 10 seconds.");
            break; // Exit the capturing loop
        }
        ...
        pass.setFilterLimits(0.0, 0.5);  // Only keep points within 0.8 meters
        pass.filter(*cloud);
