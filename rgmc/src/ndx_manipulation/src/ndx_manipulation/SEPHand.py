...
class SEPHand(Hand.Hand):

    def __init__(self, port, thumb_opp_id=2, fingers_id=1,
                 operating_mode=5, default_velocity=20, default_grasp_current=120):
        """
        ...
        self.fingers = Dynamixel.XM430_W350(motor_id=fingers_id, port_handler=port_handler, baud_rate=baud_rate)

        # Construct motor list
        motor_list = [self.thumb_opp, self.fingers]

        # Call to parent constructor
        super(SEPHand, self).__init__(motor_list)
        ...
        self.thumb_opp.set_direction(normal=True)
        self.fingers.set_direction(normal=True)
        ...
        self.fingers.set_position_I_gain(100)

        # Enable torque
        self.torque_enable()

    def calibrate(self, cal_file_path='SEP_cal.csv', min_pos_currents=30, max_pos_currents=80,
        ...
        time.sleep(2)
        self.thumb_opp.calibrate_min_pos(current=120, threshold=threshold)
        self.thumb_opp.min_pos = self.thumb_opp.min_pos - 180
        min_pos_lst.append(self.thumb_opp.min_pos)
        self.thumb_opp.torque_enable()
        self.thumb_opp.move_to_goal_position(self.thumb_opp.min_pos, tolerance=50, safe=False)

        # Calibrate thumb opposition
        print("Calibrating thumb opposition.")
        time.sleep(4)
        self.thumb_opp.calibrate_max_pos(current=-120, threshold=threshold)
        self.thumb_opp.max_pos = self.thumb_opp.max_pos + 200
        max_pos_lst.append(self.thumb_opp.max_pos)
        self.thumb_opp.torque_enable()
        self.thumb_opp.move_to_goal_position(self.thumb_opp.max_pos, tolerance=50, safe=False)

        # Put thumb into abduction
        print("Abducting thumb.")
        time.sleep(4)
        self.thumb_opp.torque_enable()
        self.thumb_opp.move_to_goal_position(self.thumb_opp.min_pos, tolerance=30)

        # Calibrate the rest of the fingers
        print("Calibrating min positions. Please ensure fingers stay in open position.")
        time.sleep(6)
        self.fingers.calibrate_min_pos(current=min_pos_currents[1], threshold=threshold)
        # Set open position
        self.fingers.torque_enable()
        self.fingers.set_goal_position(self.fingers.min_pos, safe=False)
        min_pos_lst.append(self.fingers.min_pos)
        time.sleep(2)

        print("Calibrating max positions.")
        time.sleep(4)
        # Set current and wait for steady state
        self.fingers.calibrate_max_pos(current=max_pos_currents[1], threshold=threshold)
        # Set open position
        self.fingers.torque_enable()
        self.fingers.set_goal_position(self.fingers.min_pos)
        max_pos_lst.append(self.fingers.max_pos)
        time.sleep(2)
    ...
    def grasp(self, grasp_type, max_current=None, blocking=False):
        ...
        if max_current is not None:
            self.thumb_opp.set_goal_current(max_current)
            self.fingers.set_goal_current(max_current)
        else:
            max_current = self.default_grasp_current

        # Set goal positions
        if grasp_type == 1:
            self.thumb_opp.set_goal_position(self.thumb_opp.min_pos)

        elif grasp_type == 2:
            self.thumb_opp.set_goal_position(int((self.thumb_opp.max_pos - self.thumb_opp.min_pos)*3/4 + self.thumb_opp.min_pos))

        elif grasp_type == 3:
            self.thumb_opp.set_goal_position(self.thumb_opp.max_pos)
        ...
        if blocking:
            pos = [self.thumb_opp.get_position(), self.fingers.get_position()]
            cur = [abs(self.thumb_opp.get_current()), abs(self.fingers.get_current())]
            ...
            self.thumb_opp.set_goal_position(pos[0])
            self.fingers.set_goal_position(pos[1])


    def set_finger_pos(self, position, blocking=False, position_threshold=25):
        """
        ...
        """
        if blocking:
            self.move_to_goal_position([None,position],
                                       tolerance=position_threshold)
        else:
            self.set_goal_position([None, position])
    ...
    def open(self, blocking=False, position_threshold=25):
        ...
        """
        if blocking:
            self.move_to_goal_position([None, self.fingers.min_pos],
                                       tolerance=position_threshold)
        else:
            self.set_goal_position([None, self.fingers.min_pos])
    ...
    def close(self, blocking=False, position_threshold=25):
        ...
        """
        if blocking:
            self.move_to_goal_position([None, self.fingers.max_pos],
                                       tolerance=position_threshold)
        else:
            self.set_goal_position([None, self.fingers.max_pos])
